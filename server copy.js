...
app.post("/sendDonationThankYou", async (req, res) => {
  console.log("Received request to send donation thank you email:", req.body);
  ...
  const filename = `invoice-${transactionID || Date.now()}.pdf`;

  try {
    // Generate PDF with Puppeteer (single path – no PDFKit fallback)
    ...
    // ✅ Configure the email with PDF attachment
    const mailOptions = {
      from: "Rex Vets <support@rexvets.com>",
      to: email,
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      if (error) {
        ...
        console.log("Donation thank you email sent:", info.response);
        return res.send({
          message: "Donation thank you email with invoice sent successfully!",
        });
      }
    });
  } catch (err) {
    console.error("Error creating invoice PDF:", err);
    res.status(500).send({ message: "Failed to create and send invoice" });
  }
});
