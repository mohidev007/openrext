...
export const performanceTimingMiddleware = (req, res, next) => {
  ...
  res.on("finish", () => {
    ...
    // This header cannot be set here because the response is already finished.
    // res.setHeader('X-Response-Time', `${duration.toFixed(2)}ms`);

    // Log slow requests (>1000ms)
    if (duration > 1000) {
      console.warn(
        `🐌 Slow request: ${req.method} ${req.path} took ${duration.toFixed(
          2
        )}ms`
      );
    }
...
export const memoryMonitoringMiddleware = (req, res, next) => {
  ...
  if (memUsage.rss > 512 * 1024 * 1024) {
    console.warn(
      `⚠️  High memory usage: ${(memUsage.rss / 1024 / 1024).toFixed(2)} MB`
    );
  }
...
export const requestRateLimitingMiddleware = (() => {
  ...
  return (req, res, next) => {
    ...
    if (data.count > MAX_REQUESTS) {
      console.warn(
        `🚫 Rate limit exceeded for IP: ${ip} (${data.count} requests)`
      );
      res.setHeader("X-RateLimit-Limit", MAX_REQUESTS);
      res.setHeader("X-RateLimit-Remaining", 0);
      res.setHeader(
        "X-RateLimit-Reset",
        Math.ceil((data.windowStart + WINDOW_MS) / 1000)
      );
